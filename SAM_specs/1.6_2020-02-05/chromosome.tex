% Preamble
\documentclass[11pt]{{article}}

% Packages
\usepackage{{amsmath}}
\usepackage{{fancyhdr}}
\usepackage{{verbatim}}
\usepackage{{graphicx}}
\usepackage{{float}}
\usepackage[margin=2.5cm,top=0.5cm]{{geometry}}


\begin{{document}}

\title{{Report for {read_name}}}
\date{{}}
\maketitle

% stop centering
\raggedright
\section{{{read_name} Mapping Summary}}

\noindent
\begin{{minipage}}[t]{{0.45\textwidth}}
\centering
\textbf{{Single Read Summary}} \\
\begin{{tabular}}{{|l|c|}}
\hline
\textbf{{Mapping Type}} & \textbf{{Count}} \\
\hline
Mapped & {s_mapped} \\
Partially Mapped & {s_partially_mapped} \\
Unmapped & {s_unmapped} \\
Qual $<$ {qual} & {s_low_qual} \\
\hline
Total & {s_total} \\
\hline
\end{{tabular}}
\end{{minipage}}%
\hfill
\begin{{minipage}}[t]{{0.45\textwidth}}
\centering
\textbf{{Pair Read Summary}} \\
\begin{{tabular}}{{|l|c|}}
\hline
\textbf{{Mapping Type}} & \textbf{{Count}} \\
\hline
Mapped & {p_mapped} \\
Partially Mapped & {p_partially_mapped} \\
Unmapped & {p_unmapped} \\
Qual $<$ {qual} & {p_low_qual} \\
\hline
Total & {p_total} \\
\hline
\end{{tabular}}
\end{{minipage}}

\section{{{read_name} cigar mutation observed }}
\centering
This section only include mapped and partially mapped CIGAR values since unmapped reads do not have one.
\begin{{table}}[h]
\centering
\begin{{tabular}}{{|l|c|}}
\hline
\textbf{{Mutation Type}} & \textbf{{Percentage}} \\
\hline
Alignment match & {M} \\
Insertion to the reference & {I} \\
Deletion from the reference & {D} \\
Soft clipping & {S} \\
Hard clipping & {H} \\
Skipped region & {N} \\
Padding & {P} \\
Sequence match & {X} \\
Sequence mismatch & {E} \\
\hline
\end{{tabular}}
\end{{table}}

% Insert an image
\begin{{figure}}[h]
\centering
\includegraphics[width=0.8\textwidth]{{temp/chromosome.png}}
\caption{{Mapping depth}}
\end{{figure}}

\thispagestyle{{empty}} % Suppress page number on the title page


\end{{document}}